...
function checkAnswer() {
    ...
    if (selectedAnswer === correctAnswer) {
      ...
      <img src="../images/breaking-bad-gas-mask-aftersubmit.png" id='correct_Answer' alt= 'correctAnswerIcon'/>
      <button id='nextQ'>Next</button></div>`);      
      score++;
      $('#currentScore').text(score);
    } else {
      $('.js-quiz').html(`<div class = 'badAnswer'><p>Nope! That's not Right. Correct answer is "${correctAnswer}."</p><img src= "../images/breaking-bad-wrong answer.jpg" id='wrong_Answer' alt= 'wrongAnswerIcon'/><button id='nextQ'>Next</button></div>`);
    }
...
function showResults() {
  if (score >= 7){
  $('.js-quiz').html(`<div class='resultPage'><h2>You finished the QUIZ and That's very good</h2><img src='images/finishing test.jpg' alt= 'very-good' id = 'lastImage'/><button type = 'restart' id = 'restartButton'>Restart</button></div>`);
  }  else if (score >= 4 && score <7){
      $('.js-quiz').html(`<div class='resultPage'><h2>You finished the QUIZ and That's not BAD.</h2>
      <img src='images/finishing test.jpg' alt= 'not-bad' id = 'lastImage'/>
      ...
      <img src='images/finishing test.jpg' alt= 'really-bad' id = 'lastImage'/>
      <button type = 'restart' id = 'restartButton'>Restart</button></div>`);
  }
} 
